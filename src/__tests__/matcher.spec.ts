...
const testCasesMatches: TestCase = {
  '/jobs/{id}': ['/jobs/foo', '/jobs/100', '/jobs/0'],
  '/jobs/{id}/site': ['/jobs/foo/site', '/jobs/100/site', '/jobs/0/site'],
  '/jobs/1/site': ['/jobs/1/site'],
};
...
const testCasesNotMatches: TestCase = {
  ...
  '/jobs/{id}/site': ['/jobs/foo/sites', '/v2/jobs/0/site'],
  '/jobs/1/site': ['/v2/jobs/1/site', '/jobs/1/sites'],
};
...
it('matcher matches request urls', () => {
  Object.keys(testCasesMatches).map((pattern) =>
    testCasesMatches[pattern].map((reqPath) =>
      truthyWithMessage(
        matcher(pattern, reqPath),
        `Should Match: ${pattern} === ${reqPath}`,
      ),
    ),
  );
});
...
it('matcher does not match request urls', () => {
  Object.keys(testCasesNotMatches).map((pattern) =>
    testCasesNotMatches[pattern].map((reqPath) =>
      truthyWithMessage(
        !matcher(pattern, reqPath),
        `Should Not Match: ${pattern} === ${reqPath}`,
      ),
    ),
  );
});
...
it('extracts params from path', () => {
  ...
  expect(extract('/jobs/{id}/site/{siteId}', '/jobs/101/site/3d')).toEqual({
    id: '101',
    siteId: '3d',
  });
  expect(extract('/jobs/{id}/site/{id}', '/jobs/100/site/123')).toEqual({
    id: '123',
  });
...
const endpoints: Endpoint[] = [
  {
    ...
    variants: {
      ...
      'param1=foo&param2=bar':
        '/absolutePat/foo/1/GET.param1=foo&param2=bar.json',
...
it('should return fixture path for request', () => {
  expect(
    findTestFixture(endpoints, { path: '/foo/1', method: 'GET' } as Request),
  ).toBe('/absolutePat/foo/1/GET.default.json');
});

it('should return fixture path for request with query', () => {
  expect(
    findTestFixture(endpoints, {
      path: '/foo/1',
      query: { param2: 'bar', param1: 'foo' },
      method: 'GET',
    } as Request),
  ).toBe('/absolutePat/foo/1/GET.param1=foo&param2=bar.json');
});

it('should return fixture path for request with dynamic path', () => {
  expect(
    findTestFixture(endpoints, { path: '/foo/10', method: 'GET' } as Request),
  ).toBe('/absolutePat/foo/{id}/GET.default.json');
});

it('should return false when fixture for request not found by method', () => {
  expect(
    findTestFixture(endpoints, { path: '/foo/1', method: 'PUT' } as Request),
  ).toBe(null);
});

it('should return false when fixture for request not found', () => {
  expect(
    findTestFixture(endpoints, { path: '/foo', method: 'GET' } as Request),
  ).toBe(null);
});
...
it('should return fixture variant for request', () => {
  expect(
    findTestFixture(endpoints, {
      path: '/foo/1',
      method: 'GET',
      cookies: { variants: '/foo/1/GET.unathorized,/foo/2/GET.other' },
    } as Request),
...
it('should return fixture default variant for request with unknown variant', () => {
  expect(
    findTestFixture(endpoints, {
      path: '/foo/1',
      method: 'GET',
      cookies: { variants: '/foo/1/GET.unknown' },
    } as Request),
  ).toBe('/absolutePat/foo/1/GET.default.json');
});
...
const testCases = [
  ...
  },
  {
    ...
    cookies: {
      variants:
        '/staticOverDynamicSorting/{dynamic}/GET.dynamic,/staticOverDynamicSorting/static/GET.static',
    },
...
it('extractVariant returns correct variant', () => {
  testCases.map(({ endpoint, cookies, path, method, variant }) => {
    const result = extractVariant(
      ...
      { cookies, path, method } as Request,
    );
